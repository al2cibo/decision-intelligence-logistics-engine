...
def generate_synthetic_logistics_data(
    ...
    ----------
    output_dir : Path
        Directory where the four parquet files will be written.
    n_origins : int
        Number of origin nodes to generate.
    n_destinations : int
        Number of destination nodes to generate.
    start_date : datetime
        First date in the demand history.
    end_date : datetime
        Last date in the demand history (inclusive).
    seed : int, optional
    ...
    # Base demand per destination drawn once; used only for generation, not persisted.
    base_demand = dict(
