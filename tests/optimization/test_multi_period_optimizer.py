...
class TestIntegrationEndToEnd:
    ...
    def test_forecast_extractor_to_optimizer_interface_pipeline(self):
        """Verify ForecastExtractor → OptimizerInterface → MultiPeriodResult pipeline works.

        Creates a forecast results DataFrame, extracts demand time series,
        ...
        # Create forecast results DataFrame (simulating forecasting pipeline output)
        forecast_results = pl.DataFrame(
            {
                "date": [
                    date(2024, 1, 1), date(2024, 1, 1),
                    date(2024, 1, 2), date(2024, 1, 2),
                ...
                "destination_id": ["D1", "D2"] * 3,
                "prophet_forecast": [10.0, 8.0, 12.0, 9.0, 11.0, 7.0],
            }
        )

        # Step 1: Extract demand time series using ForecastExtractor
        demand_ts = ForecastExtractor.extract_demand_time_series(
            forecast_results, "prophet_forecast"
        )

        # Verify extracted schema
        assert set(demand_ts.columns) == {"destination_id", "date", "demand"}
        assert len(demand_ts) == 6

        # Step 2: Set up optimization inputs
        origins_df = pl.DataFrame(
            {
                "origin_id": ["O1", "O2"],
                "daily_capacity": [50.0, 50.0],
            }
        )
        ...
        # Step 3: Solve using OptimizerInterface in multi mode
        ...
        # Step 4: Verify result is a valid MultiPeriodResult
        assert isinstance(result, MultiPeriodResult)
        assert result.total_cost > 0

        # Verify flows DataFrame schema
        assert set(result.flows.columns) == {"origin_id", "destination_id", "period", "flow"}
        assert result.flows.schema["origin_id"] == pl.Utf8
        assert result.flows.schema["destination_id"] == pl.Utf8
        assert result.flows.schema["period"] == pl.Date
        assert result.flows.schema["flow"] == pl.Float64

        # Verify inventory DataFrame schema
        assert set(result.inventory.columns) == {"destination_id", "period", "inventory"}
        assert result.inventory.schema["destination_id"] == pl.Utf8
        assert result.inventory.schema["period"] == pl.Date
        assert result.inventory.schema["inventory"] == pl.Float64

        # Verify inventory has one row per destination per period
        assert len(result.inventory) == 2 * 3  # 2 destinations × 3 periods

        # Verify all flows are positive (above threshold)
        assert (result.flows["flow"] > 1e-6).all()

        # Verify inventory is non-negative
        assert (result.inventory["inventory"] >= 0).all()
