...
# ---------------------------------------------------------------------------
# Property 11: Single-Period Aggregation Equivalence
# Feature: multi-period-optimization, Property 11: Single-Period Aggregation Equivalence
# ---------------------------------------------------------------------------
...
@st.composite
def single_period_aggregation_problem(draw):
    """Generate a multi-period demand problem suitable for single-period aggregation.

    Ensures:
    - All destinations are reachable (have at least one lane)
    - Total capacity >= mean demand per destination (so single-period solve is feasible)
    - All costs are non-negative, capacities are positive

    Returns a tuple of (demand_ts, origins_df, lanes_df) where demand_ts has
    schema [destination_id, date, demand] with multiple periods.
    """
    # Generate dimensions
    n_origins = draw(st.integers(min_value=1, max_value=3))
    n_destinations = draw(st.integers(min_value=1, max_value=3))
    n_periods = draw(st.integers(min_value=2, max_value=5))

    origin_ids = [f"O{i}" for i in range(n_origins)]
    destination_ids = [f"D{i}" for i in range(n_destinations)]

    # Generate planning horizon (consecutive dates)
    start_date = date(2024, 1, 1)
    planning_horizon = [start_date + timedelta(days=i) for i in range(n_periods)]

    # Generate demand values (non-negative integers to avoid floating-point issues)
    demand_dest_ids = []
    demand_dates = []
    demand_values = []
    for d_id in destination_ids:
        for dt in planning_horizon:
            demand_val = float(draw(st.integers(min_value=0, max_value=50)))
            demand_dest_ids.append(d_id)
            demand_dates.append(dt)
            demand_values.append(demand_val)
    ...
    # Compute mean demand per destination (what single-period mode will use)
    mean_demands = demand_ts.group_by("destination_id").agg(
        pl.col("demand").mean()
    )
    max_total_mean_demand = mean_demands["demand"].sum()

    # Generate lanes: connect every origin to every destination (ensures reachability)
    # Use integer costs to avoid floating-point comparison issues
    lane_origin_ids = []
    lane_dest_ids = []
    lane_costs = []
    for o_id in origin_ids:
        for d_id in destination_ids:
            lane_origin_ids.append(o_id)
            lane_dest_ids.append(d_id)
            cost = float(draw(st.integers(min_value=1, max_value=100)))
            lane_costs.append(cost)
    ...
    # Generate capacities: ensure total capacity >= max total mean demand
    # Each origin gets enough capacity so that sum of all origins >= total mean demand
    min_capacity_per_origin = (max_total_mean_demand / n_origins) + 1.0
    capacities = []
    for _ in range(n_origins):
        cap = float(draw(st.integers(
            min_value=int(min_capacity_per_origin) + 1,
            max_value=int(min_capacity_per_origin) + 100,
        )))
        capacities.append(cap)
    ...
    return demand_ts, origins_df, lanes_df


# Validates: Requirements 8.4, 8.5
@settings(max_examples=100)
@given(problem=single_period_aggregation_problem())
def test_single_period_aggregation_equivalence(problem):
    """Property 11: Single-Period Aggregation Equivalence.

    For any multi-period demand DataFrame passed to the OptimizerInterface in
    single-period mode, the optimization result SHALL be equivalent to solving
    with the Single_Period_Optimizer using the mean demand per destination
    computed from the time series.

    **Validates: Requirements 8.4, 8.5**
    """
    # Feature: multi-period-optimization, Property 11: Single-Period Aggregation Equivalence
    demand_ts, origins_df, lanes_df = problem

    # Path 1: Solve using OptimizerInterface in single-period mode with multi-period demand
    interface = OptimizerInterface(mode="single")
    result_interface = interface.solve(demand_ts, origins_df, lanes_df)

    # Path 2: Manually compute mean demand per destination, then solve with Optimizer directly
    mean_demand_df = demand_ts.group_by("destination_id").agg(
        pl.col("demand").mean()
    )
    optimizer = Optimizer()
    result_direct = optimizer.solve(mean_demand_df, origins_df, lanes_df)

    # Assert both results have the same total_cost (within floating-point tolerance)
    tolerance = 1e-6
    assert abs(result_interface.total_cost - result_direct.total_cost) < tolerance, (
        f"Total cost mismatch:\n"
        f"  OptimizerInterface (single mode): {result_interface.total_cost}\n"
        f"  Optimizer (direct with mean demand): {result_direct.total_cost}\n"
        f"  difference: {abs(result_interface.total_cost - result_direct.total_cost)}\n"
        f"  tolerance: {tolerance}"
    )
